      ...
      <div className="modal-box w-11/12 max-w-2xl bg-[#1e1e1e] border border-[#333]" onClick={(e) => e.stopPropagation()}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <div className={`badge badge-outline text-xs ${
                        codespace.state.toLowerCase() === 'available' ? 'badge-success' : 
                        codespace.state.toLowerCase() === 'starting' ? 'badge-warning' : 
                        'badge-info'
                      }`}>
                        {codespace.state}
                      </div>
          ...
          <div className="alert bg-[#2d2d2d] border-[#444] text-[#cccccc]">
            <div>
              <h3 className="font-medium flex items-center gap-2">
                Status
                {(statusText?.includes('Connecting') || statusText?.includes('Starting') || statusText?.includes('Unavailable')) && (
        ...
        <div className="modal-action bg-[#2d2d2d] p-4 rounded-b-lg -mx-6 -mb-6 mt-4">
          <button 
            ...
            disabled={statusText?.includes('Connecting') || statusText?.includes('Starting') || statusText?.includes('Getting') || statusText?.includes('Opening') || statusText?.includes('Authenticating')}
          >
            {(statusText?.includes('Connecting') || statusText?.includes('Starting') || statusText?.includes('Getting') || statusText?.includes('Opening') || statusText?.includes('Authenticating')) ? 'Connecting...' : 'Close'}
          </button>
        </div>
      </div>
    </div>
